...
def eval_epoch(model, validation_data, device):#pass
    ...
    with torch.no_grad():
        for batch in tqdm(validation_data, mininterval=2, desc='  - (Validation) ', leave=False):
            ...
            loss, n_correct = cal_performance(pred, gold, False)
...
def main():#有问题
    ...
    transformer = Transformer(opt.src_vocab_size, opt.tgt_vocab_size, opt.max_token_seq_len,
                 opt.d_word_vec, opt.d_model, opt.d_inner,
                 opt.n_layers, opt.n_head, opt.d_k, opt.d_v, opt.dropout,
